...
if out:
    ...
    with tab_preview:
        ...
        if not final_md:
            ...
            st.divider()
            st.subheader("📝 Review & Rewrite")
            feedback_text = st.text_area("What would you like to change?", placeholder="e.g., Make the tone more professional, add more detail about X, etc.")
            
            if st.button("🔄 Rewrite with Feedback"):
                if not feedback_text.strip():
                    st.warning("Please enter feedback first.")
                else:
                    # Prepare inputs for rewrite
                    # We need 'final' and 'feedback' at minimum. 
                    # Router will see 'feedback' and go to 'rewriter'.
                    inputs = {
                        "topic": out.get("topic", "blog"),
                        "final": final_md,
                        "feedback": feedback_text.strip(),
                        "as_of": out.get("as_of", date.today().isoformat()),
                        # pass along other state if available
                        "plan": out.get("plan"),
                    ...
                    for kind, payload in try_stream(app, inputs):
                        current_state = extract_latest_state(current_state, payload)
                        progress_area.json({k: v for k, v in current_state.items() if k not in ["final", "merged_md", "md_with_placeholders"]})
                    
                    st.session_state["last_out"] = current_state
                    status.update(label="✅ Rewritten!", state="complete", expanded=False)
                    st.rerun()
